...
from experiment.dataloaders import Parser
...
class ClassificationParser(Parser):
  ...
  def __init__(
    ...
    output_size : list of int
        The output_size should be divided by the largest feature stride
        `2^max_level`.
    num_classes : float
        The number of classes.
    image_field_key : str, default DEFAULT_IMAGE_FIELD_KEY
        The key name to encoded image in tf.Example.
    label_field_key : str, default DEFAULT_LABEL_FIELD_KEY
        The key name to label in tf.Example.
    decode_jpeg_only : bool, default True
        If True, only JPEG format is decoded, this is faster than decoding other
        types.
    aug_rand_hflip : bool, default True
        if True, augment training with random horizontal flip.
    aug_type : common.Augmentation, optional
        Choose from AutoAugment and RandAugment.
    color_jitter : float, default 0
        Magnitude of color jitter. If > 0, the value is used to generate random
        scale factor for brightness, contrast and saturation.
        See `preprocess_ops.color_jitter` for more details.
    random_erasing : common.RandomErasing, optional
        if not None, augment input image by random erasing. See
        `augment.RandomErasing` for more details.
    is_multilabel : bool, default False
        Whether or not each example has multiple labels.
    dtype : {'float32', 'float16', 'bfloat16'}
        Cast output image in dtype.
    """
