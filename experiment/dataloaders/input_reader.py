...
from experiment.core import input_reader
...
def calculate_batch_sizes(
  ...
  ----------
  total_batch_size : int
      The total batch size for all data.
  pseudo_label_ratio : float
      A non-negative float ratio of pseudo-labeled to labeled data in a batch.

  Returns
  -------
  tuple
      labeled_batch_size, pseudo_labeled_batch_size

  Raises
  ------
  ValueError
      If total_batch_size is negative or pseudo_label_ratio is negative.
  """
  ...
  pseudo_label_batch_size = int(round(total_batch_size * ratio_factor))
  labeled_batch_size = total_batch_size - pseudo_label_batch_size
  return labeled_batch_size, pseudo_label_batch_size


class CombinationDatasetInputReader(InputReader):
  ...
  def __init__(
    ...
    ----------
    params : DataConfig
        The parameters of the data loader.
    dataset_fn : tf.data.TFRecordDataset
        A `tf.data.Dataset` that consumes the input files.
    pseudo_label_dataset_fn : tf.data.TFRecordDataset
        A `tf.data.Dataset` that consumes the input files.
    decoder_fn : callable, optional
        An optional `callable` that takes the serialized data string and decodes
        them into the raw tensor dictionary.
    sample_fn : callable, optional
        An optional `callable` that takes a `tf.data.Dataset` object as input
        and outputs the transformed dataset. It performs sampling on the decoded
        raw tensors dict before the parser_fn.
    parser_fn : callable, optional
        An optional `callable` that takes the decoded raw tensors dict and parse
        them into a dictionary of tensors that can be consumed by the model.
        It will be executed after decoder_fn.
    transform_and_batch_fn : callable, optional
        An optional `callable` that takes a `tf.data.Dataset` object and an
        optional `tf.distribute.InputContext` as input, and returns a
        `tf.data.Dataset` object. It will be executed after `parser_fn` to
        transform and batch the dataset; if None, after `parser_fn` is executed,
        the dataset will be batched into per-replica batch size.
    postprocess_fn : callable, optional
        An optional `callable` that processes batched tensors. It will be
        executed after batching.
  ...
  def read(
    ...
    def _read_decode_and_parse_dataset(
        matched_files, dataset_fn, batch_size,
