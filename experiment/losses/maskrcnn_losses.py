...
class RPNScoreLoss(object):
  ...
  def __init__(self, rpn_batch_size_per_im: int):
  ...
  def __call__(self, score_outputs, labels):
    ...
    ----------
    score_outputs : dict
        An OrderDict with keys representing levels and values representing
        scores in [batch_size, height, width, num_anchors].
    labels : dict
        The dictionary that returned from dataloader that includes groundtruth
        targets.

    Returns
    -------
    tf.Tensor
        The total score loss.
    """
    with tf.name_scope('rpn_loss'):
      levels = sorted(score_outputs.keys())
      score_losses = []
      for level in levels:
        score_losses.append(self._rpn_score_loss(
            score_outputs[level],
            labels[level],
            normalizer=tf.cast(
  ...
  def _rpn_score_loss(self, score_outputs, score_targets, normalizer=1.0):
    ...
    with tf.name_scope('rpn_score_loss'):
      mask = tf.math.logical_or(
          tf.math.equal(score_targets, 1), tf.math.equal(score_targets, 0))
      score_targets = tf.math.maximum(
          score_targets, tf.zeros_like(score_targets))
...
class RPNBoxLoss(object):
  ...
  def __call__(self, box_outputs, labels):
    ...
    ----------
    box_outputs : dict
        An OrderDict with keys representing levels and values representing box
        regression targets in [batch_size, height, width, num_anchors * 4].
    labels : dict
        The dictionary that returned from dataloader that includes
        groundturth targets.

    Returns
    -------
    tf.Tensor
        A scalar tensor representing total box regression loss.
    """
    with tf.name_scope('rpn_loss'):
...
class FastrcnnClassLoss(object):
  ...
  def __call__(self, class_outputs, class_targets):
    ...
    Parameters
    ----------
    class_outputs : tf.Tensor
        The class prediction for each box with a shape of
        [batch_size, num_boxes, num_classes].
    class_targets : tf.Tensor
        The class label for each box with a shape of [batch_size, num_boxes].

    Returns
    -------
    tf.Tensor
        The total class loss.
    """
  ...
  def _fast_rcnn_class_loss(self,
    ...
    with tf.name_scope('fast_rcnn_class_loss'):
      class_loss = self._categorical_crossentropy(
...
class FastrcnnBoxLoss(object):
  ...
  def __init__(self,
    ...
    ----------
    huber_loss_delta : float
        The delta is typically around the mean value of regression target. for
        instances, the regression targets of 512x512 input with 6 anchors on
        P2-P6 pyramid is about [0.1, 0.1, 0.2, 0.2].
    class_agnostic_bbox_pred : bool, default False
        Whether class agnostic bounding box prediction is performed.
    """
    self._huber_loss = tf.keras.losses.Huber(
        delta=huber_loss_delta, reduction=tf.keras.losses.Reduction.SUM)
  ...
  def __call__(self, box_outputs, class_targets, box_targets):
    ...
    (Reference: https://github.com/facebookresearch/Detectron/blob/master/detectron/roi_data/fast_rcnn.py)  # pylint: disable=line-too-long
    ...
    Parameters
    ----------
    box_outputs : tf.Tensor
        The box prediction for each box with a shape of
        [batch_size, num_boxes, num_classes * 4].
    class_targets : tf.Tensor
        The class label for each box with a shape of
        [batch_size, num_boxes].
    box_targets : tf.Tensor
        The box label for each box with a shape of
        [batch_size, num_boxes, 4].

    Returns
    -------
    tf.Tensor
        The total box regression loss.
    """
    with tf.name_scope('fast_rcnn_loss'):
      class_targets = tf.cast(class_targets, dtype=tf.int32)
  ...
  def _assign_class_targets(self, box_outputs, class_targets):
    batch_size, num_rois, num_classes = box_outputs.get_shape().as_list()
    ...
    box_indices = tf.reshape(
        ...
        tf.tile(
            ...
            [1, num_rois]) +
        tf.tile(
            tf.expand_dims(tf.range(num_rois) * num_classes, 0), [batch_size, 1]
        ), [-1])
    box_outputs = tf.matmul(
        tf.one_hot(box_indices, batch_size * num_rois * num_classes,
...
class MaskrcnnLoss(object):
  ...
  def __call__(self, mask_outputs, mask_targets, select_class_targets):
    ...
    (Reference: https://github.com/facebookresearch/Detectron/blob/master/detectron/roi_data/mask_rcnn.py)  # pylint: disable=line-too-long
    ...
    Parameters
    ----------
    mask_outputs : tf.Tensor
        A float tensor representing the prediction for each mask,
        with a shape of [batch_size, num_masks, mask_height, mask_width].
    mask_targets : tf.Tensor
        A float tensor representing the binary mask of ground truth
        labels for each mask with a shape of
        [batch_size, num_masks, mask_height, mask_width].
    select_class_targets : tf.Tensor
        A tensor with a shape of [batch_size, num_masks],
