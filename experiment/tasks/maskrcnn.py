...
from experiment.common import dataset_fn
...
from experiment.evaluation import coco_evaluator
from experiment.evaluation import coco_utils
from experiment.losses import maskrcnn_losses
from experiment.model import factory


def zero_out_disallowed_class_ids(
    batch_class_ids: tf.Tensor,
  ...
  allowed_class_ids : list
      The class IDs which we want to allow.

  Returns
  -------
  tf.Tensor
      A [batch_size, num_instances] int tensor with any
      class ID not in allowed_class_ids set to 0.
  """
  allowed_class_ids = tf.constant(
      allowed_class_ids, dtype=batch_class_ids.dtype)
...
class MaskRCNNTask(base_task.Task):
  ...
  Mask R-CNN task provides artifacts for training/evaluation procedures,
  ...
  def initialize(self, model: tf.keras.Model):
    ...
    if self.task_config.init_checkpoint_modules == 'all':
      ...
      status = ckpt.read(ckpt_dir_or_file)
      status.expect_partial().assert_existing_objects_matched()
    logging.info('Finished loading pretrained checkpoint from {}'.format(
        ckpt_dir_or_file))
  ...
  def build_losses(self,
    ...
    rpn_score_loss = tf.reduce_mean(
        rpn_score_loss_fn(
            model_outputs['rpn_scores'], labels['rpn_score_targets']))
    rpn_box_loss = tf.reduce_mean(
        rpn_box_loss_fn(
            model_outputs['rpn_boxes'], labels['rpn_box_targets']))
    ...
    for cas_num in range(num_det_heads):
      frcnn_cls_loss_i = tf.reduce_mean(
          frcnn_cls_loss_fn(
              model_outputs['class_outputs_{}'.format(
                  cas_num) if cas_num else 'class_outputs'],
              model_outputs['class_targets_{}'.format(
                  cas_num) if cas_num else 'class_targets']))
      frcnn_box_loss_i = tf.reduce_mean(
          frcnn_box_loss_fn(
              model_outputs['box_outputs_{}'.format(
                  cas_num) if cas_num else 'box_outputs'],
              model_outputs['class_targets_{}'.format(
                  cas_num) if cas_num else 'class_targets'],
              model_outputs['box_targets_{}'.format(
                  cas_num) if cas_num else 'box_targets']))
    ...
    if params.model.include_mask:
      ...
      mask_class_targets = model_outputs['mask_class_targets']
  ...
  def build_metrics(self, training: bool = True):
    ...
    if training:
      ...
      if self._task_config.use_wod_metrics:
        ...
        # /docs/quick_start.md#use-pre-compiled-pippip3-packages-for-linux
        ...
        # currently used.
        try:
          from experiment.evaluation import wod_detection_evaluator  # pylint: disable=g-import-not-at-top
  ...
  def aggregate_logs(self, state, step_logs):
    if self._task_config.use_coco_metrics:
      ...
      self.coco_metric.update_state(
          step_logs[self.coco_metric.name][0],
          step_logs[self.coco_metric.name][1])
    if self._task_config.use_wod_metrics:
      if state is None:
        self.wod_metric.reset_states()
      self.wod_metric.update_state(
          step_logs[self.wod_metric.name][0],
          step_logs[self.wod_metric.name][1])
    if state is None:
