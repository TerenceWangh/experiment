...
from experiment import orbit, hyperparams
...
def get_leaf_nested_dict(d: Dict[str, Any], keys: List[str]) -> Dict[str, Any]:
  ...
  :param d: The dictionary to extract value from.
  :param keys: The list of keys to extract values recursively.
  :return: The value of the leaf.
  :raise KeyError: If the value of keys extracted is a dictionary.
  """
  leaf = d
  for k in keys:
...
def cast_leaf_nested_dict(
  ...
  :param d: The dictionary to extract value from.
  :param cast_fn: The casting function.
  :return: A dictionary with the same structure as d.
  """
  for k, v in d.items():
    if isinstance(v, dict):
      d[k] = cast_leaf_nested_dict(v, cast_fn)
    else:
      d[k] = cast_fn(v)
  return d
...
def maybe_create_best_ckpt_exporter(
  ...
  if data_dir and export_subdir and metric_name:
    ...
    best_ckpt_exporter = BestCheckpointExporter(best_ckpt_dir,
                                                metric_name,
                                                metric_comp)
    logging.info('Create the best checkpoint exporter. data_dir: %s, '
                 'export_subdir: %s. metric_name: %s.', data_dir,
...
class BestCheckpointExporter:
  ...
  def __init__(self, export_dir: str, metric_name: str, metric_comp: str):
    ...
    :param export_dir: The directory that will contain exported checkpoints.
    :param metric_name: Indicates which metric to look at, when determining
        which result is better. If eval_logs being passed to
        maybe_export_checkpoint is a nested dictionary, use `|` as a separator
        for different layers.
    :param metric_comp: Indicates how to compare results. Either `lower` or
        `higher`.
    """
    ...
    if self._metric_comp not in ('lower', 'higher'):
      raise ValueError('best checkpoint metric comp must be one of higher, '
                       'lower. Got: {}'.format(self._metric_comp))
  ...
  def _new_metric_is_better(self, old_logs, new_logs):
    ...
    if self._metric_comp == 'higher':
      if new_value > old_value:
        logging.info('[BestCheckpointExporter] the new number is better since '
                     'it is higher.')
        return True
    else:
      if new_value < old_value:
  ...
  def export_best_eval_metric(self, eval_logs, global_step):
    ...
    with tf.io.gfile.GFile(self.best_ckpt_logs_path, 'w') as writer:
      writer.write(json.dumps(eval_logs_ext, indent=2) + '\n')
...
def create_optimizer(
  ...
  if 'dp_config' in inspect.signature(task.create_optimizer).parameters:
    ...
    if hasattr(params.task, 'differential_privacy_config') and \
        params.task.differential_privacy_config is not None:
...
def create_trainer(
  ...
  return trainer_cls(
      params, task, model=model, optimizer=optimizer, train=train,
      evaluate=evaluate, checkpoint_exporter=checkpoint_exporter)
...
class ParseConfigOptions:
  ...
  def __contains__(self, item):
    return item in dataclasses.asdict(self)
...
class ExperimentParser:
  ...
  def __init__(self, flags_object):
    self._flags_object = flags_object

  def parse(self):
    """Overall process of constructing Experiment config."""
  ...
  def base_experiment(self):
    ...
    if self._flags_object.experiment is None:
      raise ValueError('The flag --experiment must be specified.')
    return exp_factory.get_exp_config(self._flags_object.experiment)

  def parse_config_file(self, params):
    """Override the configuration of params from the config_file."""
    for config_file in self._flags_object.config_file or []:
  ...
  def parse_data_service(self, params):
    """Override the data service configuration of params from flags."""
    if ('tf_data_service' in self._flags_object and
  ...
  def parse_params_override(self, params):
    ...
    if self._flags_object.params_override:
      params = hyperparams.override_params_dict(
          params, self._flags_object.params_override, is_strict=True)
    return params
...
def write_json_summary(log_dir, global_step, eval_metrics):
  ...
  with tf.io.gfile.GFile(output_json, 'w') as writer:
    writer.write(json.dumps(serializable_dict, indent=2) + '\n')
...
def write_model_params(
  ...
  with tf.io.gfile.GFile(output_path, 'w') as writer:
    ...
    for var in model.variables:
      shape = tf.shape(var)
      totoal_params += tf.math.reduce_prod(shape).numpy()
      writer.write(f'{var.name} {shape.numpy().tolist()}\n')
    writer.write(f'\nTotal params: {totoal_params}\n')
...
def try_count_params(
  ...
  :param model: Try to count the number of params in this model.
  :param trainable_only: Whether to calculate trainable params only. This flag
      is not used when the model has `count_params` attribute.
  :return: The number of parameters or None.
  """
...
def try_count_flops(
  ...
  :param model: A model instance.
  :param inputs_kwargs: An optional dictionary of argument pairs specifying
      inputs' shape specifications to getting corresponding concrete function.
  :param output_path: A file path to write the profiling results to.
  :return: The model's FLOPs.
  """
  if hasattr(model, 'inputs'):
    try:
      ...
      if output_path is not None:
        opts['output'] = 'file:outfile={}'.format(output_path)
      else:
        opts['output'] = 'none'
      ...
      logging.info('Failed to count model FLOPs with error %s, because the '
                   'build() methods in keras layers were not called. This is '
                   'probably because the model was not feed any input, e.g., '
                   'the max train step already reached before this run.', e)
    return None
