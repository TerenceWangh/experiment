...
class ExportModule(tf.Module, metaclass=abc.ABCMeta):
  ...
  def __init__(self,
    ...
    :param params: A dataclass for parameters to the module.
    :param model: A model instance which contains weights and forward
        computation.
    :param inference_step: An optional callable to forward-pass the model. If
        not specified, it creates a parital function with `model` as an required
        kwarg.
    :param preprocessor: An optional callable to preprocess the inputs.
  ...
  def serve(self) -> Mapping[Text, tf.Tensor]:
    ...
    Expecting tensors are passed in through keyword arguments.

    :return: a dictionary of tensors, when the keys will be used inside the
        SignatureDef.
    """
...
def export(export_module: ExportModule,
  ...
  :param export_module: a ExportModule with the keras Model and serving
      tf.functions.
  :param function_keys: a list of string keys to retrieve pre-defined serving
      signatures. The signature keys will be set with defaults. If a dictionary
      is provided, the values will be used as signature keys.
  :param export_saved_model_dir: Output saved model directory.
  :param checkpoint_path: Object-based checkpoint path or directory.
  :param timestamped: Whether to export the saved model to a timestamped
      directory.
  :param save_options: `SaveOptions` for `tf.saved_model.save`.
  :param checkpoint: An optional tf.train.Checkpoint. If provided, the export
      module will use it to read the weights.
  :return: The saved model directory path.
  """
  ...
  if isinstance(function_keys, list):
    if len(function_keys) == 1:
      ...
      raise ValueError('If the function_keys is a list, it must contain a '
                       'single element. {}'.format(function_keys))
...
def get_timestamped_export_dir(export_dir_base):
  ...
  :param export_dir_base: A string containing a directory to write the exported
      graph and checkpoints.
  :return: The full path of the new subdirectory (which is not actually created
      yet).
  :raise RuntimeError: if repeated attempts fail to obtain a unique timestamped
      directory name.
  """
  attempts = 0
