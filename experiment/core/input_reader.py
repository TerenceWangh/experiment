...
def _read_files_then_shard(matched_files: List[str],
  ...
  """Sends all files to every worker and then shard by data."""
...
class InputReader:
  ...
  # instances.
  static_rand_num = _get_random_integer()
  ...
  def __init__(self,
    ...
    :param params: A config_definitions.DataConfig object.
    :param dataset_fn: A `tf.data.Dataset` that consumes the input files. For
        example, it can be `tf.data.TFRecordDataset`.
    :param decoder_fn: An optional `callable` that takes the serialized data
        string and decodes them into the raw tensor dictionary.
    :param combine_fn: An optional `callable` that takes a dictionary of
    ...
    :param sample_fn: An optional `callable` that takes a `tf.data.Dataset`
        object as input and outputs the transformed dataset. It performs
        sampling on the decoded raw tensors dict before the parser_fn.
    :param parser_fn: An optional `callable` that takes the decoded raw tensors
        dict and parse them into a dictionary of tensors that can be consumed by
        the model. It will be executed after decoder_fn.
    :param transform_and_batch_fn: An optional `callable` that takes a
        ...
        batch size.
    :param postprocess_fn: A optional `callable` that processes batched tensors.
        It will be executed after batching.
    """
    ...
    if isinstance(params.input_path, Config) and combine_fn is None:
      raise ValueError(
    ...
    if not params.input_path:
      ...
      if not params.tfds_split:
        raise ValueError(
            '`tfds_name` is {}, but `tfds_split` is not specified.'.format(
                params.tfds_name))
    ...
    if self._enable_tf_data_service:
      ...
      self._tf_data_service_job_name = '{}_bs{}_{}'.format(
          params.tf_data_service_job_name,
          params.global_batch_size,
          self.static_rand_num)
  ...
  def get_files(self, input_path):
    ...
    if isinstance(input_path, Config):
      matched_files = {}
  ...
  def _maybe_apply_data_service(
    ...
    if self._enable_tf_data_service and input_context:
      if self._enable_round_robin_tf_data_service:
        ...
        dataset = range_dataset.map(
            lambda i: dataset.apply(  # pylint: disable=g-long-lambda
  ...
  def read(self,
    ...
    dataset = self._decode_and_parse_dataset(
        dataset, self._global_batch_size, input_context)
