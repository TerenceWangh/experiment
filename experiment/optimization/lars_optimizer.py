...
class LARS(tf.keras.optimizers.Optimizer):
  ...
  def __init__(self,
    ...
    :param momentum:  hyper parameter >= 0 that accelerates gradient descent
    in the relevant direction and dampens oscillations. Defaults to 0.9.
    :param weight_decay: weight decay.
    :param eeta: LARS coefficient as used in the paper. Default set to LARS
    coefficient from the paper. (eeta / weight_decay) determines the highest
    scaling factor in LARS..
    :param nesterov: whether to use nesterov momentum.
    :param classic_momentum: whether to use classic (or popular) momentum.
    The learning rate is applied during momentum update in classic momentum,
    but after momentum for popular momentum.
    :param exclude_from_weight_decay: A list of `string` for variable
    screening, if any of the string appears in a variable's name,
    the variable will be excluded for computing weight decay. For example,
    one could specify the list like ['batch_normalization', 'bias'] to
    exclude BN and bias from weight decay.
    :param exclude_from_layer_adaptation: Similar to
    exclude_from_weight_decay, but for layer adaptation. If it is None,
    it will be defaulted the same as exclude_from_weight_decay.
    :param name: `Text` as optional name for the operations created when
    applying gradients. Defaults to "LARS".
    ...
    # exclude_from_lay_adaptation is set to exclude_from_weight_decay if the
    # arg is None.
  ...
  def _do_layer_adaptation(self, param_name):
    """Whether to do layer-wise laerning rate adaptation for `param_name`."""
