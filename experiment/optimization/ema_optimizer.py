...
class ExponentialMovingAverage(tf.keras.optimizers.Optimizer):
  ...
  Examples of usage for training:
  ...
  def __init__(
      ...
      **kwargs):
    """Construct a new ExponentialMovingAverage optimize

    :param optimizer: tf.keras.optimizers.Optimizer` that will be used to
    compute and apply gradients. 'bool', if True, only model trainable weights
    will be updated. Otherwise, all model weights will be updated. This mainly
    affects batch normalization parameters.
    :param trainable_weights_only: Decay to use to maintain the moving averages
    of trained variables.
    :param average_decay: Decay to use maintain the moving average op trained
    variables.
    :param start_step: What step to start the moving average.
    :param dynamic_decay: Whether to change the decay based on the number of
    ...
    :param kwargs: keyword arguments. Allowed to bve {'clipnorm', 'clipvalue',
    'lr', 'decay'}.
    """
  ...
  def update_average(self, step: tf.Tensor):
    ...
    return ctx.merge_cell(_update, args=(zip(self._average_weights,
                                             self._model_weights)))
  ...
  def _swap_weights(self):
    ...
    def swap(strategy, a_and_b):
      """Swap `a` and `b` and mirror to all deivces."""
      for a, b in a_and_b:
    ...
    ctx = tf.distribute.get_replica_context()
    return ctx.merge_cell(swap, args=zip(self._average_weights,
                                         self._model_weights))

  def assign_average_vars(self, var_list: List[tf.Variable]):
    """Assign variables in `var_list` with their respective averages.

    :param var_list: List of model variables to be assigned to their average.
    :return: The op corresponding to the assignment operation of variables to
    their average.
    """
    assign_op = tf.group([
  ...
  @iterations.setter
  def iterations(self, variables):
    self._optimizer.iterations = variables
