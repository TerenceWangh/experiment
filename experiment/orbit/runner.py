...
class AbstractTrainer(tf.Module, metaclass=abc.ABCMeta):
  ...
  def train(self, num_steps: tf.Tensor) -> Optional[Output]:
    ...
    :param num_steps: The number of training steps to run. Note that it is up to
        the model what constitutes a "step", which may involve more than one
        update to model parameters (e.g., if training a GAN).
    :return: Either `None`, or a dictionary mapping names to `Tensor`s or NumPy
        values. If a dictionary is returned, it will be written to logs and as
        TensorBoard summaries. The dictionary may also be nested, which will
        generate a hierarchy of summary directories.
    """
    pass
...
class AbstractEvaluator(tf.Module, metaclass=abc.ABCMeta):
  ...
  def evaluate(self, num_steps: tf.Tensor) -> Optional[Output]:
    ...
    :param num_steps: The number of evaluation steps to run. Note that it is up
        to the model what constitutes a "step". Evaluations may also want to
