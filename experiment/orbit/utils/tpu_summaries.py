...
class OptionalSummariesFunction:
  ...
  :attribute with_summaries: A wrapped version of the underlying function with
      summaries enabled (using whatever the active predicate is for
      `tf.summary.record_if`), and placed inside a "soft device placement"
      context to enable summary recording on TPU.
  :attribute without_summaries: A wrapped version of the underlying function
      with all summary recording disabled.
  """
  ...
  def __init__(self, function, **tf_function_kwargs):
    ...
    :param function: The underlying function to wrap.
    :param tf_function_kwargs: Additional arguments to pass to `tf.function`.
    """
  ...
  def __get__(self, instance, owner):
    ...
    :return: A new bound instance of `TpuDiscretionarySummariesFunctions`.
    """
