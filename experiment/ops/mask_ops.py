...
def paste_instance_masks(masks,
  ...
  ----------
  masks : np.ndarray
      A numpy array of shape [N, mask_height, mask_width] representing the
      instance masks w.r.t. the `detected_boxes`.
  detected_boxes : np.ndarray
      A numpy array of shape [N, 4] representing the reference bounding boxes.
  image_height : int
  ...
  -------
  np.ndarray
      A numpy array of shape [N, image_height, image_width] representing
  ...
  scale = max((mask_w + 2.) / mask_w,
              (mask_h + 2.) / mask_h)

  ref_boxes = expand_boxes(detected_boxes, scale)
  ref_boxes = ref_boxes.astype(np.int32)
  padded_mask = np.zeros((mask_h + 2, mask_w + 2), dtype=np.float32)
  segms = []
