...
def _feature_bilinear_interpolation(features, kernel_y, kernel_x):
  ...
  .. math:: f(y, x) = [hy, ly] * [[f00, f01], [f10, f11]] * [hx, lx]^T
  ...
  ----------
  features : tf.Tensor
      The features are in shape of [batch_size, num_boxes, output_size *
      2, output_size * 2, num_filters].
  kernel_y : tf.Tensor
      Tensor of size [batch_size, boxes, output_size, 2, 1].
  kernel_x : tf.Tensor
      Tensor of size [batch_size, boxes, output_size, 2, 1].
...
def _compute_grid_positions(boxes, boundaries, output_size, sample_offset):
  ...
  ----------
  boxes : tf.Tensor
      A 3-D tensor of shape [batch_size, num_boxes, 4] encoding the information
      of each box w.r.t. the corresponding feature map. boxes[:, :, 0:2] are the
      grid position in (y, x) (float) of the top-left corner of each box.
      boxes[:, :, 2:4] are the box sizes in (h, w) (float) in terms of the
      number of pixels of the corresponding feature map size.
  boundaries : tf.Tensor
      A 3-D tensor of shape [batch_size, num_boxes, 2] representing the boundary
      (in (y, x)) of the corresponding feature map for each box. Any resampled
      grid points that go beyond the bounary will be clipped.
  output_size : int
      A scalar indicating the output crop size.
  sample_offset : float
      A float number in [0, 1] indicates the subpixel sample offset from grid
      point.
  ...
  kernel_x : tf.Tensor
      Tensor of size [batch_size, boxes, output_size, 2, 1].
  box_grid_y0y1 : tf.Tensor
      Tensor of size [batch_size, boxes, output_size, 2].
  box_grid_x0x1 : tf.Tensor
      Tensor of size [batch_size, boxes, output_size, 2].
  """
...
def multilevel_crop_and_resize(
  ...
  ----------
  features : dict
      A dictionary with key as pyramid level and value as features. The
      features are in shape of [batch_size, height_l, width_l, num_filters].
  boxes : tf.Tensor
      A 3-D Tensor of shape [batch_size, num_boxes, 4]. Each row represents
      a box with [y1, x1, y2, x2] in un-normalized coordinates.
  output_size : int
      A scalar to indicate the output crop size.
  sample_offset : float
      A float number in [0, 1] indicates the subpixel sample offset from grid
      point.
  ...
  with tf.name_scope('multilevel_crop_and_resize'):
    ...
    # TODO: replace tf.gather with tf.gather_nd and try to get similar
    # performance.
...
def _selective_crop_and_resize(
    features,
    boxes,
    box_levels,
    boundaries,
  ...
  ----------
  features : tf.Tensor
      A 5-D tensor of shape [batch_size, num_levels, max_height, max_width,
      num_filters] where cropping and resizing are based.
  boxes : tf.Tensor
      A 3-D tensor of shape [batch_size, num_boxes, 4] encoding the information
      of each box w.r.t. the corresponding feature map. boxes[:, :, 0:2] are the
      grid position in (y, x) (float) of the top-left corner of each box.
  ...
  box_levels : tf.Tensor
      A 3-D tensor of shape [batch_size, num_boxes, 1] representing the 0-based
      corresponding feature level index of each box.
  boundaries : tf.Tensor
      A 3-D tensor of shape [batch_size, num_boxes, 2] representing the boundary
      (in (y, x)) of the corresponding feature map for each box. Any resampled
      grid points that go beyond the bounary will be clipped.
  output_size : int, default 7
      A scalar indicating the output crop size.
  sample_offset : float, default 0.5
      A float number in [0, 1] indicates the subpixel sample offset from grid
      point.
  use_einsum_gather : bool, default False
      Use einsum to replace gather or not. Replacing einsum with gather can
      improve performance when feature size is not large, einsum is friendly
      with model partition as well. Gather's performance is better when feature
      size is very large and there are multiple box levels.

  Returns
  -------
  tf.Tensor
      A 5-D tensor of shape [batch_size, num_boxes, output_size, output_size,
      num_filters] representing the cropped features.
  """
...
def crop_mask_in_target_box(
  ...
  target_boxes : tf.Tensor
      A float tensor representing target box cooridnates for masks with a shape
      of [batch_size, num_masks, 4] in un-normalized coordinates. A box is
      represented by [ymin, xmin, ymax, xmax].
  output_size : int
      A scalar to indicate the output crop size. It currently only supports to
      output a square shape outputs.
  sample_offset : float, default 0
      A float number in [0, 1] indicates the subpixel sample offset from grid
      point.
...
def nearest_upsampling(data, scale, use_keras_layer=False):
  ...
  ----------
  data : tf.Tensor
      A tensor with a shape of [batch, height_in, width_in, channels].
  scale : int
      An integer multiple to scale resolution of input data.
  use_keras_layer : bool, default False
      If True, use keras Upsampling2D layer.

  Returns
  -------
  tf.Tensor
      A tensor with a shape of [batch, height_in*scale, width_in*scale,
      channels]. Same dtype as input data.
  """
  if use_keras_layer:
