...
class BalancedPositiveNegativeSampler:
  ...
  def __init__(self, positive_fraction: float = 0.5, is_static: bool = False):
    """Constructs a minibatch sampler.

    Parameters
    ----------
    positive_fraction : float, default 0.5
        The desired fraction of positive examples (scalar in [0,1])
        in the batch.
    is_static : bool, default False
        Whether uses an implementation with static shape guarantees.
    """
    if positive_fraction < 0 or positive_fraction > 1:
      raise ValueError('positive_fraction should be in range [0, 1]. '
                       'Received: {}.'.format(positive_fraction))
    self._positive_fraction = positive_fraction
    self._is_static = is_static

  @staticmethod
  def subsample_indicator(indicator: tf.Tensor, num_samples: tf.Tensor):
  ...
  def _get_num_pos_neg_samples(self, sorted_indices_tensor, sample_size):
    ...
    Parameters
    ----------
    sorted_indices_tensor : tf.Tensor
        A sorted int32 tensor of shape [N] which contains the signed indices of
        the examples where the sign is based on the label value. The examples
        that cannot be sampled are set to 0. It samples at most
        `sample_size*positive_fraction` positive examples and remaining
        from negative examples.
    sample_size : tf.Tensor
        The size of subsamples.

    Returns
    -------
    tf.Tensor
        The number of positive labels in the subsample.
    tf.Tensor
        The number of negative labels in the subsample.
    """
    input_length = tf.shape(input=sorted_indices_tensor)[0]
    valid_positive_indices = tf.greater(
        sorted_indices_tensor, tf.zeros(input_length, tf.int32))
    num_sampled_pos = tf.reduce_sum(
        input_tensor=tf.cast(valid_positive_indices, tf.int32))
    max_num_positive_samples = tf.constant(
        int(sample_size * self._positive_fraction), tf.int32)
    sample_size = tf.constant(sample_size, tf.int32)
  ...
  def _get_values_from_start_and_end(self,
                                     input_tensor,
                                     num_start_samples,
                                     num_end_samples,
                                     total_num_samples):
    ...
    input_tensor : tf.Tensor
        An int32 tensor of shape [N] to be sliced.
    num_start_samples : tf.Tensor
        Number of examples to be sliced from the beginning of the input tensor.
    num_end_samples : tf.Tensor
        Number of examples to be sliced from the end of the input tensor.
    total_num_samples : tf.Tensor
        Sum of is num_start_samples and num_end_samples. This should be a
        scalar.
  ...
  def _static_subsample(self, indicator, batch_size, labels):
    ...
    ----------
    indicator : tf.Tensor
        A boolean tensor of shape [N] whose True entries can be sampled.
        N should be a compile time constant.
    batch_size : tf.Tensor
        The desired batch size.
    labels : tf.Tensor
        A boolean tensor of shape [N] denoting positive(=True) and negative
        (=False) examples. N should be a compile time constant.


    Returns
    -------
    tf.Tensor
        A boolean tensor of shape [N], True for entries which are sampled. It
        ensures the length of output of the subsample is always batch_size, even
        when number of examples set to True in indicator is less than
        batch_size.
    """
    ...
    if not labels.shape.is_fully_defined():
      raise ValueError(
          'labels must be static in shape when is_static is True.')
    ...
    sampled_idx_selected = tf.greater_equal(sampled_idx, tf.constant(0))
    sampled_idx_selected = tf.cast(sampled_idx_selected, tf.int32)
    sampled_idx = tf.multiply(sampled_idx_selected, sampled_idx)

    sampled_idx_indicator = tf.cast(
        tf.reduce_sum(input_tensor=tf.one_hot(sampled_idx, depth=input_length),
                      axis=0), tf.bool)
  ...
  def subsample(self, indicator, batch_size, labels, scope=None):
    ...
    batch_size : tf.Tensor or int
        The desired batch size. If None, keeps all positive samples and
    ...
    -------
    tf.Tensor
        A boolean tensor of shape [N], True for entries which are sampled.
    """
    if len(indicator.get_shape().as_list()) != 1:
      raise ValueError('indicator must be 1 dimensional, got a tensor of '
                       'shape {}'.format(indicator.get_shape()))
    if len(labels.get_shape().as_list()) != 1:
      raise ValueError('labels must be 1 dimensional, got a tensor of '
                       'shape {}'.format(labels.get_shape()))
    if labels.dtype != tf.bool:
      raise ValueError('labels should be of type bool. Received: {}'.format(
          labels.dtype))
    if indicator.dtype != tf.bool:
      raise ValueError('indicator should be of type bool. Received: {}'.format(
          indicator.dtype))
    ...
    with tf.name_scope(scope):
      ...
      if batch_size is None:
        ...
        negative_positive_ratio = negative_fraction / self._positive_fraction
        max_num_neg = tf.cast(negative_positive_ratio * tf.cast(
            num_sampled_pos, dtype=tf.float32))
      else:
        max_num_pos = batch_size - num_sampled_pos
