...
def to_4d(image: tf.Tensor) -> tf.Tensor:
  ...
  :param image: The 2/3/4D input tensor.
  :return: A 4D image tensor.
  :raise TypeError: if `image` is not a 2/3/4D tensor.
  """
...
def from_4d(image: tf.Tensor, ndims: tf.Tensor) -> tf.Tensor:
  ...
  :param image: A 4D image tensor
  :param ndims: the dimensions of the `image`.
  :return: n-d tensors.
  """
  shape = tf.shape(image)
...
def _convert_translation_to_transform(translations: tf.Tensor) -> tf.Tensor:
  ...
  :param translations: The 2-element list representing [dx, dy], or a matrix of
  2-element lists representing [dx, dy] to translate for each image. The shape
  must be static.
  :return: The transformation matrix of shape (num_images, 8).
  :raise TypeError: when the shape of `translations` is not known or not rank
  1 or 2.
  """
...
def _convert_angles_to_transform(angles: tf.Tensor,
                                 image_width: tf.Tensor,
                                 image_height: tf.Tensor) -> tf.Tensor:
  """Converts an angle or angles to a projective transform.

  :param angles: A scalar to rotate all images, or a vector to rotate a batch of
  images. This must be a scalar.
  :param image_width: The width of the image(s) to be transformed.
  :param image_height: The height of the image(s) to be transformed.
  :return: A tensor of shape (num_images, 8).
  :raise TypeError: when `angles` is not rank 0 or 1.
  """
...
def translate(image: tf.Tensor, translations) -> tf.Tensor:
  ...
  :param image: An image Tensor of type uint8.
  :param translations: A vector or matrix representing [dx dy].
  :return: The translated version of the image.
  """
...
def rotate(image: tf.Tensor, degrees: float) -> tf.Tensor:
  ...
  :param image: An image Tensor of type uint8.
  :param degrees: A scalar angle in degrees to rotate all images by. If
  degrees is positive the image will be rotated clockwise otherwise it will
  be rotated counterclockwise.
  :return: The rotated version of image.
  """
  # Convert from degrees to radians.
  degree_to_radians = math.pi / 180.0
  radians = tf.cast(degrees * degree_to_radians, tf.float32)
  original_ndims = tf.rank(image)
  image = to_4d(image)
...
def blend(image1: tf.Tensor, image2: tf.Tensor, factor: float) -> tf.Tensor:
  ...
  interpolate the pixel values between the two images.  A value greater than
  1.0 "extrapolates" the difference between the two pixel values, and we clip
  the results to values between 0 and 255.

  :param image1: An image Tensor of type uint8.
  :param image2: An image Tensor of type uint8.
  :param factor: A floating point value above 0.0.
  :return: A blended image Tensor of type uint8.
  """
  if factor == 0.0:
  ...
  if factor > 0.0 and factor < 1.0:
    # Interpolate means we always stay within 0 and 255.
...
def cutout(image: tf.Tensor, pad_size: int, replace: int = 0) -> tf.Tensor:
  ...
  location within `image`. The pixel values filled in will be of the value
  `replace`. The location where the mask will be applied is randomly chosen
  uniformly over the whole image.

  :param image: An image Tensor of type uint8.
  :param pad_size: Specifies how big the zero mask that will be generated is
  that is applied to the image. The mask will be of size (2*pad_size x
  2*pad_size).
  :param replace: What pixel value to fill in the image in the area that has
  the cutout mask applied to it.
  :return: An image Tensor that is of type uint8.
  """
...
def _fill_rectangle(image,
  ...
  lo_pad = tf.maximum(0, center_height - half_height)
  up_pad = tf.maximum(0, image_h - center_height - half_height)
  lt_pad = tf.maximum(0, center_width - half_width)
...
def cutout_video(image: tf.Tensor, replace: int = 0) -> tf.Tensor:
  ...
  :param image: A video Tensor of type uint8.
  :param replace: What pixel value to fill in the image in the area that has
  the cutout mask applied to it.
  :return: An video Tensor that is of type uint8.
  """
  image_d = tf.shape(image)[0]
  ...
  lt_pad = tf.maximum(0, center_w - pad_w)
  rt_pad = tf.maximum(0, image_w - center_w - pad_w)
  bk_pad = tf.maximum(0, center_d - pad_d)
...
def auto_contrast(image: tf.Tensor) -> tf.Tensor:
  """Implements Autocontrast function from PIL using TF ops.

  :param image: A 3D uint8 tensor.
  :return: The image after it has had auto contrast applied to it and will be
  of type uint8.
  """
...
def unwrap(image: tf.Tensor, replace: int) -> tf.Tensor:
  ...
  Operations like translate and shear on a wrapped Tensor will leave 0s in
  empty locations.  Some transformations look at the intensity of values to
  do preprocessing, and we want these empty pixels to assume the 'average'
  value, rather than pure black.
...
def _apply_bbox_augmentation(image, bbox, augmentation_func, *args):
  ...
  :param image: 3D uint8 Tensor.
  :param bbox: 1D Tensor that has 4 elements (min_y, min_x, max_y, max_x)
  of type float that represents the normalized coordinates between 0 and 1.
  :param augmentation_func: Augmentation function that will be applied to the
  subsection of image.
  :param *args: Additional parameters that will be passed into augmentation_func
  when it is called.
  :return: A modified version of image, where the bbox location in the image
  will have `ugmentation_func applied to it.
  """
...
def _apply_bbox_augmentation_wrapper(image, bbox, new_bboxes, prob,
  ...
  :param image: 3D uint8 Tensor.
  :param bbox: 1D Tensor that has 4 elements (min_y, min_x, max_y, max_x) of
  type float that represents the normalized coordinates between 0 and 1.
  :param new_bboxes: 2D Tensor that is a list of the bboxes in the image after
  they have been altered by aug_func. These will only be changed when
  func_changes_bbox is set to true. Each bbox has 4 elements (min_y, min_x,
  max_y, max_x) of type float that are the normalized bbox coordinates between
  0 and 1.
  :param prob: Float that is the probability of applying
  _apply_bbox_augmentation.
  :param augmentation_func: Augmentation function that will be applied to the
  subsection of image.
  :param func_changes_bbox: Boolean. Does augmentation_func return bbox in
  addition to image.
  :param *args: Additional parameters that will be passed into augmentation_func
  when it is called.
  :return: Fist element is a modified version of image, where the bbox location
  in the image will have augmentation_func applied to it if it is chosen to be
  called with probability `prob`. The second element is a Tensor of Tensors
  of length 4 that will contain the altered bbox after applying
  augmentation_func.
  """
...
def _apply_multi_bbox_augmentation(image, bboxes, prob, aug_func,
  ...
  :param image: 3D uint8 Tensor.
  :param bboxes: 2D Tensor that is a list of the bboxes in the image. Each bbox
  has 4 elements (min_y, min_x, max_y, max_x) of type float.
  :param prob: Float that is the probability of applying aug_func to a specific
  bounding box within the image.
  :param aug_func: Augmentation function that will be applied to the subsections
  of image indicated by the bbox values in bboxes.
  :param func_changes_bbox: Boolean. Does augmentation_func return bbox in
  addition to image.
  :param *args: Additional parameters that will be passed into augmentation_func
  when it is called.
  :return: A modified version of image, where each bbox location in the image
  will have augmentation_func applied to it if it is chosen to be called with
  probability prob independently across all bboxes. Also the final bboxes are
  returned that will be unchanged if func_changes_bbox is set to false and if
  true, the new altered ones will be returned.
  :raise ValueError: if applied to video.
  """
  if image.shape.rank == 4:
...
def _clip_bbox(min_y, min_x, max_y, max_x):
  ...
  :return: Clipped coordinate values between 0 and 1.
  """
...
def _check_bbox_area(min_y, min_x, max_y, max_x, delta=0.05):
  ...
  bbox min/max coordinates that are the same on the boundary. This prevents
  the bbox from having an area of zero.
  :return: Tuple of new bbox coordinates between 0 and 1 that will now have a
  guaranteed area > 0.
  """
  height = max_y - min_y
  width = max_x - min_x
...
def _rotate_bbox(bbox, image_height, image_width, degrees):
  ...
  :param degrees: Float, a scalar angle in degrees to rotate all images by. If
  degrees is positive the image will be rotated clockwise otherwise it will be
  rotated counterclockwise.
  :return: A tensor of the same shape as bbox, but now with the rotated
  coordinates.
  """
...
def rotate_with_bboxes(image, bboxes, degrees, replace):
  ...
  rotated counterclockwise.
  :param replace: A one or three value 1D tensor to fill empty pixels.
  :return: A tuple containing a 3D uint8 Tensor that will be the result of
  rotating image by degrees. The second element of the tuple is bboxes,
  where now the coordinates will be shifted to reflect the rotated image.
  :raise ValueError: If applied to video.
  """
  if image.shape.rank == 4:
    raise ValueError('Image rank 4 is not supported')
...
def _shear_bbox(bbox, image_height, image_width, level, shear_horizontal):
  ...
  :param image_width: height of the image.
  :param level: How much to shear the image.
  :param shear_horizontal: If true then shear in X dimension else shear in
  the Y dimension.
  :return: A tensor of the same shape as bbox, but now with the shifted
  coordinates.
  """
  image_height, image_width = (
    tf.cast(image_height, tf.float32), tf.cast(image_width, tf.float32))
...
def shear_with_bboxes(image, bboxes, level, replace, shear_horizontal):
  ...
  :param image: 3D uint8 Tensor.
  :param bboxes: 2D Tensor that is a list of the bboxes in the image. Each bbox
  has 4 elements (min_y, min_x, max_y, max_x) of type float with values between
  [0, 1].
  :param level: Float. How much to shear the image. This value will be between
  -0.3 to 0.3.
  :param replace: A one or three value 1D tensor to fill empty pixels.
  :param shear_horizontal: Boolean. If true then shear in X dimension else
  shear in the Y dimension.
  :return: A tuple containing a 3D uint8 Tensor that will be the result of
  shearing image by level. The second element of the tuple is bboxes, where now
...
def _shift_bbox(bbox, image_height, image_width, pixels, shift_horizontal):
  ...
  :param pixels: An int. How many pixels to shift the bbox.
  :param shift_horizontal: Boolean. If true then shift in X dimension else
...
def translate_bbox(image, bboxes, pixels, replace, shift_horizontal):
  ...
  has 4 elements (min_y, min_x, max_y, max_x) of type float with values between
  [0, 1].
  :param pixels: An int. How many pixels to shift the image and bboxes
  :param replace: A one or three value 1D tensor to fill empty pixels.
  :param shift_horizontal: If true then shift in X dimension else shift in
  Y dimension.
  :return: A tuple containing a 3D uint8 Tensor that will be the result of
  translating image by pixels. The second element of the tuple is bboxes,
  where now the coordinates will be shifted to reflect the shifted image.
  :raise ValueError: if applied to video.
  """
  if image.shape.rank == 4:
    raise ValueError('Image rank 4 is not supported')
...
class ImageAugment(object):
  ...
  def distort(
    ...
    height, width, 3] representing an image or image sequence.
    :return: The augmented version of `image`.
    """
  ...
  def distort_with_boxes(
    ...
    :param image: `Tensor` of shape [height, width, 3] or [num_frames,
    height, width, 3] representing an image or image sequence.
    :param bboxes: `Tensor` of shape [num_boxes, 4] or [num_frames, num_boxes,
    4] representing bounding boxes for an image or image sequence.
    :return: The augmented version of `image` and `bboxes`.
    """
    raise NotImplementedError
...
class AutoAugment(ImageAugment):
  ...
  def __init__(
    ...
    :param augmentation_name: The name of the AutoAugment policy to use. The
    available options are `v0`, `test`, `reduced_cifar10`, `svhn` and
    `reduced_imagenet`. `v0` is the policy used for all of the results in the
    paper and was found to achieve the best results on the COCO dataset.
    `v1`, `v2` and `v3` are additional good policies found on the COCO
    dataset that have slight variation in what operations were used during
    the search procedure along with how many operations are applied in
    parallel to a single image (2 vs 3). Make sure to set `policies` to
    `None` (the default) if you want to set options using `augmentation_name`.
    :param policies: list of lists of tuples in the form `(func, prob, level)`,
    ...
    sub-policy can vary from one sub-policy to another. If you provide
    `policies` as input, any option set with `augmentation_name` will get
    overridden as they are mutually exclusive.
    :param cutout_const: multiplier for applying cutout.
    :param translate_const: multiplier for applying translation.
    Raises:
  ...
  def _check_policy_shape(self, policies):
    ...
    :param policies: List of list of tuples in the form `(func, prob, level)`.
    Must have shape of `(:, :, 3)`.
    :raise ValueError: if the shape of `policies` is unexpected.
    """
...
class RandAugment(ImageAugment):
  ...
  def __init__(self,
    ...
    :param num_layers: Integer, the number of augmentation transformations to
    apply sequentially to an image. Represented as (N) in the paper. Usually
    best values will be in the range [1, 3].
    :param magnitude: Integer, shared magnitude across all augmentation
    operations. Represented as (M) in the paper. Usually best values are in
    the range [5, 10].
    :param cutout_const: multiplier for applying cutout.
    :param translate_const: multiplier for applying translation.
    :param magnitude_std: randomness of the severity as proposed by the authors
    of the timm library.
    :param prob_to_apply: The probability to apply the selected augmentation at
    each layer.
    :param exclude_ops: exclude selected operations.
    """
...
class RandomErasing(ImageAugment):
  ...
  def __init__(self,
    ...
    :param probability (float, optional): Probability of augmenting the image.
    Defaults to 0.25.
    :param min_area (float, optional): Minimum area of the random erasing
    rectangle. Defaults to 0.02.
    :param max_area (float, optional): Maximum area of the random erasing
    rectangle. Defaults to 1/3.
    :param min_aspect (float, optional): Minimum aspect rate of the random
    erasing rectangle. Defaults to 0.3.
    :param max_aspect ([type], optional): Maximum aspect rate of the random
    erasing rectangle. Defaults to None.
    :param min_count (int, optional): Minimum number of erased rectangles.
    Defaults to 1.
    :param max_count (int, optional):  Maximum number of erased rectangles.
    Defaults to 1.
    :param trials (int, optional): Maximum number of trials to randomly
    sample a rectangle that fulfills constraint. Defaults to 10.
    """
  ...
  def distort(self, image: tf.Tensor) -> tf.Tensor:
    ...
    :param image: Of shape [height, width, 3] representing an image.
    :return: tf.Tensor: The augmented version of `image`.
    """
...
class MixupAndCutmix:
  ...
  def __init__(self,
    ...
    :param mixup_alpha: For drawing a random lambda (`lam`) from a beta
    distribution (for each image). If zero Mixup is deactivated. Defaults to .8.
    :param cutmix_alpha: For drawing a random lambda (`lam`) from a beta
    distribution (for each image). If zero Cutmix is deactivated. Defaults to 1.
    :param prob: Of augmenting the batch. Defaults to 1.0.
    :param switch_prob: Probability of applying Cutmix for the batch.
    Defaults to 0.5.
    :param label_smoothing: Constant for label smoothing. Defaults to 0.1.
    :param num_classes: Number of classes. Defaults to 1001.
    """
  ...
  def distort(self, images: tf.Tensor,
    ...
    :param images: Of shape [batch_size,height, width, 3] representing a
    batch of image.
    :param labels: Of shape [batch_size, ] representing the class id for each
    image of the batch.
