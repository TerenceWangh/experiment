...
def yxyx_to_xywh(boxes):
  ...
  ----------
  boxes : np.ndarray
      A numpy array whose last dimension is 4 representing the coordinates
...
def yxyx_to_cycxhw(boxes):
  ...
  ----------
  boxes : tf.Tensor
      A `Tensor` with last dimension of 4, representing the coordinates of
      boxes in ymin, xmin, ymax, xmax order.

  Returns
  -------
  tf.Tensor
      A `Tensor` with the same shape as the inputted boxes, in the format
...
def jitter_boxes(boxes, noise_scale=0.025):
  ...
  boxes : tf.Tensor
      ...
      boxes in ymin, xmin, ymax, xmax order.
  noise_scale : float, default 0.025
      A python float which specifies the magnitude of noise. The rule of thumb
      is to set this between (0, 0.1]. The default value is found to mimic the
      noisy detections best empirically.

  Returns
  -------
  tf.Tensor
      A tensor whose shape is the same as `boxes` representing the jittered
      boxes.
...
def normalize_boxes(boxes, image_shape):
  ...
  ----------
  boxes : tf.Tensor
      A tensor whose last dimension is 4 representing the coordinates of boxes
      in `ymin, xmin, ymax, xmax` order.
  image_shape : list of int
      A list of two integers, a two-element vector or a tensor such that all but
      the last dimensions are `broadcastable` to `boxes`. The last dimension
      is 2, which represents [height, width].

  Returns
  -------
  tf.Tensor
      A tensor whose shape is the same as `boxes` representing the normalized
      boxes.
...
def denormalize_boxes(boxes, image_shape):
  ...
  -------
  tf.Tensor
      A tensor whose shape is the same as `boxes` representing the denormalized
      boxes.
...
def clip_boxes(boxes, image_shape):
  ...
  -------
  tf.Tensor
      A tensor whose shape is the same as `boxes` representing the clipped
      boxes.
...
def compute_outer_boxes(boxes, image_shape, scale=1.0):
  ...
  image_shape : list of int
      ...
      is 2, which represents [height, width].
  scale : float
      A float number specifying the scale of output outer boxes to input
      `boxes`.

  Returns
  -------
  tf.Tensor
      A tensor whose shape is the same as `boxes` representing the outer boxes.
  """
  if scale < 1.0:
    raise ValueError(
...
def encode_boxes(boxes, anchors, weights=None):
  ...
  -------
  tf.Tensor
      A tensor whose shape is the same as `boxes` representing the encoded box
      targets.
...
def decode_boxes(encoded_boxes, anchors, weights=None):
  ...
  encoded_boxes : tf.Tensor
      A tensor whose last dimension is 4 representing the coordinates of encoded
      boxes in `ymin, xmin, ymax, xmax` order.
  anchors : tf.Tensor
      A tensor whose shape is the same as, or `broadcastable` to `boxes`,
      representing the coordinates of anchors in ymin, xmin, ymax, xmax order.
  ...
  -------
  tf.Tensor
      A tensor whose shape is the same as `boxes` representing the decoded box
      targets.
  """
...
def filter_boxes(boxes, scores, image_shape, min_size_threshold):
  ...
  image_shape : tf.Tensor
      A tensor whose shape is the same as, or `broadcastable` to `boxes` except
      the last dimension, which is 2, representing [height, width] of the scaled
      image.
  min_size_threshold : float
      The minimal box size in each side (w.r.t. the scaled image). Boxes whose
      sides are smaller than it will be filtered out.

  Returns
  -------
  filtered_boxes : tf.Tensor
      A tensor whose shape is the same as `boxes` but with the position of the
      filtered boxes are filled with 0.
  filtered_scores : tf.Tensor
      A tensor whose shape is the same as `scores` but with the positinon of the
      filtered boxes filled with 0.
  """
  if boxes.shape[-1] != 4:
    raise ValueError(
        'boxes.shape[1] is {:d}, but must be 4.'.format(boxes.shape[-1]))
...
def filter_boxes_by_scores(boxes, scores, min_score_threshold):
  ...
  boxes : tf.Tensor
      ...
      of boxes in `ymin, xmin, ymax, xmax` order.
  scores : tf.Tensor
      A tensor whose shape is the same as tf.shape(boxes)[:-1] representing the
      original scores of the boxes.
  min_score_threshold : float
      The minimal box score threshold. Boxes whose score are smaller than it
  ...
  filtered_boxes : tf.Tensor
      A tensor whose shape is the same as `boxes` but with the position of the
      filtered boxes are filled with -1.
  filtered_scores : tf.Tensor
      A tensor whose shape is the same as 'scores' but with the positinon of the
      filtered boxes filled with 0.
  """
...
def gather_instances(selected_indices, instances, *aux_instances):
  ...
  selected_indices : tf.Tensor
      A Tensor of shape [batch, K] which indicates the selected indices in
      instance dimension (2nd dimension).
  instances : tf.Tensor
      A Tensor of shape [batch, N, ...] where the 2nd dimension is the instance
      dimension to be selected from.
  *aux_instances : tf.Tensor
      The additional Tensors whose shapes are in [batch, N, ...] which are the
      tensors to be selected from using the `selected_indices`.

  Returns
  -------
  selected_instances : tf.Tensor
      The tensor of shape [batch, K, ...] which corresponds to the selected
      instances of the `instances` tensor.
  selected_aux_instances : tf.Tensor
      The additional tensors of shape [batch, K, ...] which corresponds to the
      selected instances of the `aus_instances` tensors.
  """
...
def top_k_boxes(boxes, scores, k):
  ...
  ----------
  boxes : tf.Tensor
      A tensor of shape [batch_size, N, 4] representing the coordinate of
      the boxes. N is the number of boxes per image.
  scores : tf.Tensor
      A tensor of shsape [batch_size, N] representing the socre of the boxes.
  k : int
      An integer or a tensor indicating the top k number.

  Returns
  -------
  selected_boxes : tf.Tensor
      A tensor of shape [batch_size, k, 4] representing the selected top k box
      coordinates.
  selected_scores : tf.Tensor
      A tensor of shape [batch_size, k] representing the selected top k box
      scores.
  """
...
def bbox_overlap(boxes, gt_boxes):
  ...
  gt_boxes : tf.Tensor
      A tensor with a shape of [batch_size, MAX_NUM_INSTANCES, 4]. This tensor
      might have paddings with a negative value.
...
def box_matching(boxes, gt_boxes, gt_classes):
  ...
  ----------
  boxes : tf.Tensor
      A tensor of shape of [batch_size, N, 4] representing the box coordiantes
      to be matched to groundtruth boxes.
  gt_boxes : tf.Tensor
      A tensor of shape of [batch_size, MAX_INSTANCES, 4] representing the
      groundtruth box coordinates. It is padded with -1s to indicate the invalid
      boxes.
  gt_classes : tf.Tensor
      [batch_size, MAX_INSTANCES] representing the groundtruth box classes.
      It is padded with -1s to indicate the invalid classes.

  Returns
  -------
  matched_gt_boxes : tf.Tensor
      A tensor of shape of [batch_size, N, 4], representing the matched
      groundtruth box coordinates for each input box. If the box does not
      overlap with any groundtruth boxes, the matched boxes of it will be
      set to all 0s.
  matched_gt_classes : tf.Tensor
      A tensor of shape of [batch_size, N], representing the matched groundtruth
      classes for each input box. If the box does not overlap with any
      groundtruth boxes, the matched box classes of it will be set to 0, which
      corresponds to the background class.
  matched_gt_indices : tf.Tensor
      A tensor of shape of [batch_size, N], representing the indices of the
      matched groundtruth boxes in the original gt_boxes tensor. If the box
      does not overlap with any groundtruth boxes, the index of the matched
      groundtruth will be set to -1.
  matched_iou : tf.Tensor
      A tensor of shape of [batch_size, N], representing the IoU between the box
      and its matched groundtruth box. The matched IoU is the maximum IoU of the
      box and all the groundtruth boxes.
  iou : tf.Tensor
      A tensor of shape of [batch_size, N, K], representing the IoU matrix
