...
from experiment.ops import augment
...
def compute_padded_size(desired_size, stride):
  ...
  Parameters
  ----------
  desired_size : tf.Tensor or list of int
      Two elements representing [height, width] of the target output image size.
  stride : int
      The stride of the backbone network.

  Returns
  -------
  tf.Tensor or list of int
      Two elements representing [height, width] of the padded output image size.
  """
...
def resize_and_crop_image(
  ...
  desired_size : tf.Tensor or list of int
      Two elements representing [height, width] of the desired actual output
      image size.
  padded_size : tf.Tensor or list of int
      Two elements representing [height, width] of the padded output image size.
      Padding will be applied after scaling the image to the desired_size.
  aug_scale_min : float
      The value with range between [0, 1.0] representing minimum random scale
      applied to desired_size for training scale jittering.
  aug_scale_max : float
      The value with range between [1.0, inf] representing maximum random scale
      applied to desired_size for training scale jittering.
  seed : int, default 1
      Seed for random scale jittering.
  method : tf.image.ResizeMethod
      The function to resize input image to scaled image.

  Returns
  -------
  output_image : tf.Tensor
      `Tensor` of shape [height, width, 3] where [height, width] equals to
      `output_size`.
  image_info : tf.Tensor
      A 2D `Tensor` that encodes the information of the image and the applied
      ...
      and [y_scale, x_scale] is the scaling factor, which is the ratio of
...
def resize_and_crop_image_v2(
  ...
  image : tf.Tensor
      A `Tensor` of shape [height, width, 3] representing an image.
  short_side : tf.Tensor or int
      The desired short side to be rescaled to.
  long_side : tf.Tensor or int
      The desired long side to be rescaled to.
  padded_size : tf.Tensor or list of int
      Two elements representing [height, width] of the padded output image size.
      Padding will be applied after scaling the image to the desired_size.
  aug_scale_min : float
      The minimum random scale applied to desired_size for training scale
      jittering.
  aug_scale_max : float
      The maximum random scale applied to desired_size for training scale
      jittering.
  seed : int, optional'
      The seed for random scale jittering.
  method : tf.image.ResizeMethod
      The function to resize input image to scaled image.
...
def resize_image(
  ...
  size : tuple or int
      If list to tuple, resize to it. If scalar, we keep the same aspect ratio
      and resize the short side to the value.
  max_size : int, optional
      Only used when size is a scalar. When the larger side is larger than
      max_size after resized with size we used max_size to keep the aspect
      ratio instead.
  method : tf.image.ResizeMethod
  ...
  image_info : tf.Tensor
      ...
      [[original_height, original_width],
       ...
       [y_scale, x_scale],
       [0, 0]],
      where [resized_height, resized_width] is the actual scaled image size, and
      [y_scale, x_scale] is the scaling factor, which is the ratio of scaled
      dimension / original dimension.
  """
...
def random_crop_image(
  ...
  image : tf.Tensor
      A Tensor of shape [height, width, 3] representing the input image.
  aspect_ratio_range : list of float, default (3. / 4., 4. / 3.)
      The cropped area of the image must have an aspect `ratio = width / height`
      within this range.
  area_range : list of float, default (0.08, 1.0)
      The cropped reas of the image must contain a fraction of the input image
      within this range.
  max_attempts : int, default 10
      The number of attempts at generating a cropped region of the image of the
      specified constraints. After max_attempts failures, return the entire
      image.
  seed : int, default 1
...
def random_crop_image_v2(
  ...
  area_range : list of float, default (0.08, 1.0)
      The cropped reas of the image must contain a fraction of the input image
      within this range.
  max_attempts : int, default 10 the number of attempts at generating a cropped
  ...
  -------
  tf.Tensor
      A Tensor representing the random cropped image. Can be the original image
...
def resize_and_crop_boxes(
  ...
  ----------
  boxes : tf.Tensor
      `Tensor` of shape [N, 4] representing ground truth boxes.
  image_scale : tf.Tensor
      2D float `Tensor` representing scale factors that apply to
      [height, width] of input image.
  output_size : tf.Tensor
      2D `Tensor` or `int` representing [height, width] of target output image
      size.
  offset : tf.Tensor
      2D `Tensor` representing top-left corner [y0, x0] to crop scaled boxes.

  Returns
  -------
  tf.Tensor
      `Tensor` of shape [N, 4] representing the scaled boxes.
  """
...
def resize_and_crop_masks(
  ...
  ----------
  masks : tf.Tensor
      `Tensor` of shape [N, H, W, 1] representing ground truth masks.
  image_scale : tf.Tensor
      2D float `Tensor` representing scale factors that apply to
      [height, width] of input image.
  output_size : tf.Tensor
      2D `Tensor` or `int` representing [height, width] of target
      output image size.
  offset : tf.Tensor
      2D `Tensor` representing top-left corner [y0, x0] to crop scaled boxes.
  ...
  -------
  tf.Tensor
      `Tensor` of shape [N, H, W, 1] representing the scaled masks.
  """
...
def color_jitter(
  ...
  ----------
  image : tf.Tensor
      Tensor of shape [height, width, 3] and type uint8.
  brightness : float, default 0
      Magnitude for brightness jitter.
  contrast : float, default 0
      Magnitude for contrast jitter.
  saturation : float, default 0
      Magnitude for saturation jitter.
  seed : int, optional
      Random seed.
...
def random_crop_image_with_boxes_and_labels(
    img, boxes, labels, min_scale,
    aspect_ratio_range, min_overlap_params, max_retry):
  ...
  min_scale : float
      The lower bound of the random scale variable.
  aspect_ratio_range : list of float
      Specifies the lower and upper bound of the random aspect ratio.
  min_overlap_params : list of float
      The min value, max value, step size, and offset for the minimum overlap
      sample.
  max_retry : int
      The number of trials for cropping. If it is exhausted, no cropping will be
      performed.

  Returns
  -------
  img : tf.Tensor
      The random cropped image. Can be the original image if max_retry is
      exhausted.
  boxes : tf.Tensor
      The bounding boxes in the cropped image.
  labels : tf.Tensor
      The new bounding boxes' labels.
  """
...
def random_crop(
  ...
  labels : tf.Tensor
      A 'Tensor' of shape [N,] representing the class labels of the boxes.
  min_scale : float, default 0.3
      The lower bound of the random scale variable.
  aspect_ratio_range : list of float, default (0.5, 2.0)
      Specifies the lower and upper bound of the random aspect ratio.
  min_overlap_params : list of float, default (0.0, 1.4, 0.2, 0.1)
      The min value, max value, step size, and offset for the minimum overlap
      sample.
  max_retry : int, default 50
      The number of trials for cropping. If it is exhausted, no cropping will be
      performed.
  seed : int, optional
