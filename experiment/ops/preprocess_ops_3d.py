...
def _sample_or_pad_sequence_indices(
    sequence: tf.Tensor,
    num_steps: int,
    stride: int,
...
function can be used in evaluation in order to extract enough segments to span
  ...
  -------
  tf.Tensor
      A single `tf.Tensor` with first dimension `num_windows` * `num_steps`.
      The tensor contains the concatenated list of `num_windows` tensors which
      offsets have been linearly spaced from input.
  """
  sequence_length = tf.shape(sequence)[0]
...
def decode_jpeg(image_string: tf.Tensor, channels: int = 0) -> tf.Tensor:
  ...
  Parameters
  ----------
  image_string : tf.Tensor
      A `tf.Tensor` of type strings with the raw JPEG bytes where the first
      dimension is timesteps.
  channels : int, default 0
      Number of channels of the JPEG image. Allowed values are 0, 1 and 3.
      If 0, the number of channels will be calculated at runtime and no static
      shape is set.
...
def crop_image(
  ...
  -------
  tf.Tensor
      A Tensor of shape [timesteps, out_height, out_width, channels] of type
      uint8 with the cropped images.
  """
  if random:
    # Random spatial crop.
...
def resize_smallest(
  ...
  ----------
  frames : tf.Tensor
      A Tensor of dimension [timesteps, input_h, input_w, channels].
  min_resize : int
      Minimum size of the final image dimensions.
...
def random_crop_resize(
  ...
  output_w : int
      Resized image width.
  num_frames : int
      Number of input frames per clip.
  num_channels : int
      Number of channels of the clip.
  aspect_ratio : tuple
      Float tuple with the aspect range for cropping.
  area_range : tuple
      Float tuple with the area range for cropping.
