...
from experiment.utils.object_detection import MinibatchSampler
...
class BalancedPositiveNegativeSampler(MinibatchSampler):
  ...
  def __init__(self, positive_fraction = 0.5, is_static = False):
    ...
    positive_fraction: float, default 0.5.
        Desired fraction of positive examples (scalar in [0,1]) in the batch.
    is_static: bool, default False.
        If True, uses an implementation with static shape guarantees.

    Raises
    ------
        ValueError: if positive_fraction not in range [0, 1].
    """
    super(BalancedPositiveNegativeSampler, self).__init__()
  ...
  def _get_num_pos_neg_samples(self, sorted_indices_tensor, sample_size):
    ...
    sorted_indices_tensor: tf.Tensor.
        A sorted int32 tensor of shape [N] which contains
  ...
  def _get_values_from_start_and_end(self,
    ...
    start_pos = tf.less(tf.range(input_length), num_start_samples)
    end_pos = tf.greater_equal(tf.range(input_length),
                               input_length - num_end_samples)
    selected_pos = tf.logical_or(start_pos, end_pos)
    selected_pos = tf.cast(selected_pos, tf.float32)
    indices_pos = tf.multiply(tf.cumsum(selected_pos), selected_pos)
    one_hot_selector = tf.one_hot(
        tf.cast(indices_pos, tf.int32) - 1,
        total_num_samples,
        dtype=tf.float32)
    return tf.cast(
        tf.tensordot(
  ...
  def _static_subsample(self, indicator, batch_size, labels):
    ...
    batch_size: int.
        Desired batch size. This scalar cannot be None.
    labels: tf.Tensor.
    ...
    Returns
    -------
        Boolean tensor of shape [N], True for entries which are sampled.
        It ensures the length of output of the subsample is always batch_size,
        even when number of examples set to True in indicator is less than
        batch_size.
    ...
    # Set the number of examples et True in indicator to be at least batch_size.
    ...
    [n_pos_samples, n_neg_samples] = self._get_num_pos_neg_samples(
        sorted_signed_indicator_idx, batch_size)
    sampled_idx = self._get_values_from_start_and_end(
        sorted_signed_indicator_idx, n_pos_samples, n_neg_samples, batch_size)
  ...
  def subsample(self, indicator, batch_size, labels, scope = None):
    ...
    indicator: tf.Tensor.
        Boolean tensor of shape [N] whose True entries can be sampled.
    batch_size: int.
        Desired batch size. If None, keeps all positive samples and randomly
        selects negative samples so that the positive sample fraction matches
        self._positive_fraction. It cannot be None if is_static is True.
    labels: tf.Tensor
        Boolean tensor of shape [N] denoting positive(=True) and negative
        (=False) examples.
    scope: str, optional.
    ...
    if len(indicator.get_shape().as_list()) != 1:
      raise ValueError(
          'indicator must be 1 dimensional, got a tensor of shape {}.'.format(
              indicator.get_shape()))
    if len(labels.get_shape().as_list()) != 1:
      raise ValueError(
          'labels must be 1 dimensional, got a tensor of shape {}.'.format(
              labels.get_shape()))
    if indicator.dtype != tf.bool:
      raise ValueError(
          'indicator should be of type bool. Received: {}'.format(
              indicator.dtype))
    if labels.dtype != tf.bool:
      raise ValueError(
          'labels should be of type bool. Received: {}'.format(labels.dtype))

    with tf.name_scope(scope or 'BalancedPositiveNegativeSampler'):
      if self._is_static:
      ...
      if batch_size is None:
        negative_positive_ratio = (1. - self._positive_fraction) / \
                                  self._positive_fraction
        max_num_neg = tf.cast(
            negative_positive_ratio * tf.cast(num_sampled_pos,
                                              dtype=tf.float32),
            dtype=tf.int32)
      else:
