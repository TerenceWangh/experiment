...
class Match:
  ...
  def __init__(self, match_results):
    ...
    match_results: tf.Tensor.
        Integer tensor of shape [N] with:
    ...
    ------
    ValueError
        If match_results does not have rank 1 or is not an integer int32
        scalar tensor
    """
  ...
  def matched_column_indices(self):
    ...
    -------
    tf.Tensor
        int32 tensor of shape [K] with column indices.
    """
  ...
  def num_unmatched_columns(self):
    ...
    -------
    tf.int32
        The number of unmatched columns.
    """
  ...
  def ignored_column_indicator(self):
    ...
    -------
    tf.Tensor
        boolean vector which is True for all ignored column indices.
    """
  ...
  def matched_row_indices(self):
    ...
    -------
    tf.Tensor
        int32 tensor of shape [K] with row indices.
    """
    return self._reshape_and_cast(
        tf.gather(self._match_results, self.matched_column_indices()))

  def _reshape_and_cast(self, tensor):
    return tf.cast(tf.reshape(tensor, [-1]), tf.int32)
  ...
  def gather_based_on_match(self, input_tensor, unmatched_value, ignored_value):
    ...
    input_tensor: tf.Tensor
        Tensor to gather values from.
    unmatched_value: tf.Tensor
        Constant tensor value for unmatched columns.
    ignored_value: tf.Tensor
        Constant tensor value for ignored columns.

    Returns
    -------
    tf.Tensor
        A tensor containing values gathered from input_tensor.
...
class Matcher:
  ...
  def match(self, similarity_matrix, scope=None, **params):
    ...
    similarity_matrix: tf.Tensor
        Float tensor of shape [N, M] with pairwise similarity where higher value
        means more similar.
    scope: str, optional
        Op scope name. Defaults to 'Match' if None.
    params: dict
    ...
    """
    with tf.name_scope(scope or 'Match'):
  ...
  def _match(self, similarity_matrix, **params):
    ...
    -------
    tf.Tensor
        Integer tensor of shape [M]: match_results[i]>=0 means that column i is
        matched to row match_results[i], match_results[i]=-1 means that the
        column is not matched. match_results[i]=-2 means that the column is
        ignored (usually this happens when there is a very weak match which one
        neither wants as positive nor negative example).
    """
