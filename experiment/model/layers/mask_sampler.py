...
def _sample_and_crop_foreground_masks(candidate_rois: tf.Tensor,
  ...
  candidate_rois : tf.Tensor
      A `tf.Tensor` of shape of [batch_size, N, 4], where N is the number of
      candidate RoIs to be considered for mask sampling. It includes both
      positive and negative RoIs. The `num_mask_samples_per_image` positive
  ...
  candidate_gt_classes : tf.Tensor
      A `tf.Tensor` of shape of [batch_size, N], storing the corresponding
      groundtruth classes to the `candidate_rois`. 0 in the tensor corresponds
      to the background class, i.e. negative RoIs.
  candidate_gt_indices : tf.Tensor
      A `tf.Tensor` of shape [batch_size, N], storing the corresponding
      groundtruth instance indices to the `candidate_gt_boxes`,
  ...
  gt_masks : tf.Tensor
      A `tf.Tensor` of [batch_size, MAX_INSTANCES, mask_height,  mask_width]
      containing all the groundtruth masks which sample masks are drawn from.
  num_sampled_masks : int, default 128
      The number of masks to sample.
  mask_target_size : int, default 28
      The final cropped mask size after sampling. The output masks are resized
      w.r.t the sampled RoIs.
  ...
  batch_indices = tf.expand_dims(
      tf.range(fg_instance_indices_shape[0]), axis=-1) * tf.ones(
  ...
  batch_indices = tf.expand_dims(
      tf.range(foreground_gt_indices_shape[0]), axis=-1) * tf.ones(
...
class MaskSampler(tf.keras.layers.Layer):
  ...
  def call(self,
    ...
    candidate_rois : tf.Tensor
        ...
        RoIs will be sampled to create mask training targets.
    candidate_gt_boxes : tf.Tensor
        A `tf.Tensor` of shape of [batch_size, N, 4], storing the corresponding
        groundtruth boxes to the `candidate_rois`.
    candidate_gt_classes : tf.Tensor
        A `tf.Tensor` of shape of [batch_size, N], storing the corresponding
        groundtruth classes to the `candidate_rois`. 0 in the tensor corresponds
        to the background class, i.e. negative RoIs.
    ...
    -------
    tuple
        foreground_rois: A `tf.Tensor` of shape of [batch_size, K, 4] storing
        the RoI that corresponds to the sampled foreground masks, where
        K = num_mask_samples_per_image.
        foreground_classes: A `tf.Tensor` of shape of [batch_size, K] storing
        the classes corresponding to the sampled foreground masks.
        cropped_foreground_masks: A `tf.Tensor` of shape of
  ...
  def get_config(self):
    config = super(MaskSampler, self).get_config()
    config.update({
        'mask_target_size': self._mask_target_size,
        'num_sampled_masks': self._num_sampled_masks,
    })
    return config
