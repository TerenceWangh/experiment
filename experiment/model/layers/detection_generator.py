...
from experiment.ops import nms
...
def _generate_detections_v1(
  ...
  ----------
  boxes : tf.Tensor
      A `tf.Tensor` with shape `[batch_size, N, num_classes, 4]` or
      `[batch_size, N, 1, 4]` for box predictions on all feature levels. The
      N is the number of total anchors on all levels.
  scores : tf.Tensor
      A `tf.Tensor` with shape `[batch_size, N, num_classes]`, which
      stacks class probability on all feature levels. The N is the number of
      total anchors on all levels. The num_classes is the number of classes
      predicted by the model. Note that the class_outputs here is the raw score.
  attributes : dict, optional
      None or a dict of (attribute_name, attributes) pairs. Each
  ...
  pre_nms_top_k : int, default 5000
      The number of top candidate detections per class before NMS.
  pre_nms_score_threshold : float, default 0.05
      The threshold for deciding when to remove boxes based on score.
  nms_iou_threshold : float, default 0.5
      The threshold for deciding whether boxes overlap too much with respect to
      IoU.
  max_num_detections : int, default 100
      The maximum number of boxes retained over all classes.
  soft_nms_sigma : float, optional
      The sigma parameter for Soft NMS.
      When soft_nms_sigma=0.0 (which is default), we fall back to standard NMS.

  Returns
  -------
  tf.Tensor
      A `float` type `tf.Tensor` of shape `[batch_size, max_num_detections, 4]`
      representing top detected boxes in `[y1, x1, y2, x2]`.
  tf.Tensor
      A `float` type `tf.Tensor` of shape `[batch_size, max_num_detections]`
      representing sorted confidence scores for detected boxes. The values are
      between `[0, 1]`.
  tf.Tensor
      An `int` type `tf.Tensor` of shape `[batch_size, max_num_detections]`
      representing classes for detected boxes.
  tf.Tensor
      An `int` type `tf.Tensor` of shape `[batch_size]` only the top
      `valid_detections` boxes are valid detections.
  dict
      None or a dict of (attribute_name, attributes). Each attribute is a
      `float` type `tf.Tensor` of shape
  ...
  with tf.name_scope('generate_detections'):
    ...
    for i in range(batch_size):
      detections = _generate_detections_per_image(
          boxes[i], scores[i], attributes={
      ...
      nmsed_boxes.append(detections[0])
      nmsed_scores.append(detections[1])
      nmsed_classes.append(detections[2])
      valid_detections.append(detections[3])
      if attributes:
        for att_name in attributes.keys():
          nmsed_attributes[att_name].append(detections[4][att_name])

    boxes = tf.stack(nmsed_boxes, axis=0)
    scores = tf.stack(nmsed_scores, axis=0)
    classes = tf.stack(nmsed_classes, axis=0)
    detections = tf.stack(valid_detections, axis=0)
    if attributes:
      for att_name in attributes.keys():
        nmsed_attributes[att_name] = tf.stack(
            nmsed_attributes[att_name], axis=0)

    return boxes, scores, classes, detections, nmsed_attributes
...
def _generate_detections_per_image(
  ...
  ----------
  boxes : tf.Tensor
      A  `tf.Tensor` with shape `[N, num_classes, 4]` or `[N, 1, 4]`, which
      box predictions on all feature levels. The N is the number of total
      anchors on all levels.
  scores : tf.Tensor
      A `tf.Tensor` with shape `[N, num_classes]`, which stacks class
      probability on all feature levels. The N is the number of total anchors on
      all levels. The num_classes is the number of classes predicted by the
      model. Note that the class_outputs here is the raw score.
  attributes : dict, optional
      If not None, a dict of `tf.Tensor`. Each value is in shape
  ...
  max_num_detections : int, default 100
      The maximum number of boxes retained over all classes.
  soft_nms_sigma : float, optional
      The sigma parameter for Soft NMS. When soft_nms_sigma=0.0, we fall back to
      standard NMS. If set to None, `tf.image.non_max_suppression_padded` is
      called instead.

  Returns
  -------
  tf.Tensor
      A `float` tf.Tensor of shape `[max_num_detections, 4]`
      representing top detected boxes in `[y1, x1, y2, x2]`.
  tf.Tensor
      A `float` tf.Tensor of shape `[max_num_detections]` representing sorted
      confidence scores for detected boxes. The values are between [0, 1].
  tf.Tensor
      An `int` tf.Tensor of shape `[max_num_detections]` representing classes
      for detected boxes.
  tf.Tensor
      An `int` tf.Tensor of shape [1] only the top `valid_detections` boxes are
      valid detections.
  dict
      Each value is a `float` tf.Tensor of shape
  ...
  return nmsed_boxes, nmsed_scores, nmsed_classes, \
         valid_detections, nmsed_attributes
...
def _select_top_k_scores(scores_in: tf.Tensor, pre_nms_num_detections: int):
  ...
  pre_nms_num_detections : int
      The number of candidates before NMS.

  Returns
  -------
  tf.Tensor
      The scores with shape
      `[batch_size, pre_nms_num_detections, num_classes]`.
  tf.Tensor
      The indices with shape
      `[batch_size, pre_nms_num_detections, num_classes]`.
  """
  batch_size, num_anchors, num_classes = scores_in.get_shape().as_list()
  ...
  scores = tf.transpose(top_k_scores, [0, 2, 1])
...
def _generate_detections_v2(boxes: tf.Tensor,
  ...
  -------
  tf.Tensor
      A `float` tf.Tensor of shape [batch_size, max_num_detections, 4]
      representing top detected boxes in [y1, x1, y2, x2].
  tf.Tensor
      A `float` tf.Tensor of shape [batch_size, max_num_detections]
      representing sorted confidence scores for detected boxes. The values are
      between [0, 1].
  tf.Tensor
      An `int` tf.Tensor of shape [batch_size, max_num_detections]
      representing classes for detected boxes.
  tf.Tensor
      An `int` tf.Tensor of shape [batch_size] only the top
      `valid_detections` boxes are valid detections.
  """
  with tf.name_scope('generate_detections'):
    nmsed_boxes = []
...
def _generate_detections_batched(boxes: tf.Tensor,
                                 scores: tf.Tensor,
  ...
  scores : tf.Tensor
      ...
      predicted by the model. Note that the class_outputs here is the raw score.
  pre_nms_score_threshold : float
      A `float` representing the threshold for deciding when to remove boxes
      based on score.
  nms_iou_threshold : float
...
def _generate_detections_tflite_implements_signature(config):
  ...
  ----------
  config : dict
      A dictionary of configs defining parameters for TFLite NMS op.
...
def _generate_detections_tflite(raw_boxes: Mapping[str, tf.Tensor],
  ...
  ----------
  raw_boxes : dict
      A dictionary of tensors for raw boxes. Key is level of features
      and value is a tensor denoting a level of boxes with shape
      `[1, H, W, 4 * num_anchors]`.
  raw_scores : dict
      A dictionary of tensors for classes. Key is level of features
      and value is a tensor denoting a level of logits with shape
      `[1, H, W, num_class * num_anchors]`.
  anchor_boxes : dict
      A dictionary of tensors for anchor boxes. Key is level of
  ...
  -------
  tuple
      A tuple containing boxes, scores, classes, num_detections.
  """
  scores, boxes, anchors = [], [], []
  levels = list(raw_scores.keys())
  min_level, max_level = int(min(levels)), int(max(levels))
  ...
  for i in range(min_level, max_level + 1):
    scores.append(tf.sigmoid(tf.reshape(
        raw_scores[str(i)], [batch_size, -1, num_classes])))
  ...
  anchors = tf.stack([y_center_a, x_center_a, ha, wa], axis=-1)
...
class DetectionGenerator(tf.keras.layers.Layer):
  ...
  def __init__(self,
    ...
    Parameters
    ----------
    apply_nms : bool, default True
        Whether or not apply non maximum suppression. If False, the decoded
        boxes and their scores are returned.
    pre_nms_top_k : int, default 5000
        The number of top scores proposals to be kept before applying NMS.
    pre_nms_score_threshold : float, default 0.05
        The score threshold to apply before applying NMS. Proposals whose
        scores are below this threshold are thrown away.
    nms_iou_threshold : float, default 0.5
        The NMS IoU threshold.
    max_num_detections : int, default 100
        The final number of total detections to generate.
    nms_version : str, default 'v2'
        A string of `batched`, `v1` or `v2` specifies NMS version.
    use_cpu_nms : bool, default False
        Whether or not enforce NMS to run on CPU.
    soft_nms_sigma : float, optional
        The sigma parameter for Soft NMS.
    ...
    self._use_cpu_nms = use_cpu_nms
    self._soft_nms_sigma = soft_nms_sigma
  ...
  def __call__(self,
    ...
    ----------
    raw_boxes : tf.Tensor
        The class-specific box coordinates relative to anchors.
    raw_scores : tf.Tensor
        The class logits before applying score activiation.
    anchor_boxes : tf.Tensor
        The corresponding anchor boxes w.r.t `box_outputs`.
    image_shape : tf.Tensor
        The image height and width w.r.t. the scaled image, i.e. the same image
        space as `box_outputs` and `anchor_boxes`.
    regression_weights : list of float, optional
        A list of four float numbers to scale coordinates.
    bbox_per_class : bool, default True
        Whether perform per-class box regression.
    ...
    dict
        If `apply_nms` = True, the return is a dictionary with keys:
          ...
          `detection_scores`: A `float` `tf.Tensor` of shape
            [batch, max_num_detections] representing sorted confidence scores
        ...
        If `apply_nms` = False, the return is a dictionary with keys:
          `decoded_boxes`: A `float` tf.Tensor of shape
            [batch, num_raw_boxes, 4]
            representing all the decoded boxes.
          `decoded_box_scores`: A `float` tf.Tensor of shape
            [batch, num_raw_boxes] representing scores of all the decoded boxes.
    """
    ...
    # Optionally force the NMS be run CPU.
    if self._use_cpu_nms:
      ...
      if self._nms_version == 'batched':
        ...
        nmsed_boxes, nmsed_scores, nmsed_classes, valid_detections, _ = \
            _generate_detections_v1(
                ...
                soft_nms_sigma=self._soft_nms_sigma)
      elif self._nms_version == 'v2':
        nmsed_boxes, nmsed_scores, nmsed_classes, valid_detections = \
  ...
  def get_config(self):
    config = super(DetectionGenerator, self).get_config()
    config.update({
        ...
        'nms_iou_threshold': self._nms_iou_threshold,
        'max_num_detections': self._max_num_detections,
        'nms_version': self._nms_version,
        'use_cpu_nms': self._use_cpu_nms,
        'soft_nms_sigma': self._soft_nms_sigma,
    })
    return config
...
class MultiLevelDetectionGenerator(tf.keras.layers.Layer):
  ...
  def __init__(self,
    ...
    soft_nms_sigma : float, optional
        The sigma parameter for Soft NMS.
        When soft_nms_sigma=0.0, we fall back to standard NMS.
    tflite_post_processing_config : dict, optional
        An optional dictionary containing post-processing parameters used for
        TFLite custom NMS op.
    kwargs : dict
        Additional keyword arguments passed to Layer.
    """
    self._apply_nms = apply_nms
    ...
    self._use_cpu_nms = use_cpu_nms
    self._soft_nms_sigma = soft_nms_sigma
    self._tflite_post_processing_config = tflite_post_processing_config
  ...
  def _decode_multi_level_outputs(
    ...
    for i in range(min_level, max_level + 1):
      ...
      _, hi, wi, num_anchors = raw_boxes_i.get_shape().as_list()
      num_locations = hi * wi
      num_anchors = num_anchors // 4
      num_classes = raw_scores_i.get_shape().as_list()[-1] // num_anchors

      # Applies score transformation and remove the implicit background class.
      scores_i = tf.sigmoid(
          tf.reshape(raw_scores_i, [
              batch_size, num_locations * num_anchors, num_classes]))
      ...
      anchor_boxes_i = tf.reshape(
          anchor_boxes[str(i)], [batch_size, num_locations * num_anchors, 4])
      raw_boxes_i = tf.reshape(
          raw_boxes_i, [batch_size, num_locations * num_anchors, 4])
      ...
      if raw_attributes:
        for att_name, raw_att in raw_attributes.items():
          attribute_size = raw_att[str(i)].get_shape().as_list()[-1]
          attribute_size = attribute_size // num_anchors
          att_i = tf.reshape(raw_att[str(i)], [
              batch_size, num_locations * num_anchors, attribute_size])
  ...
  def __call__(self,
    ...
    dict:
        ...
        If `apply_nms` = False, the return is a dictionary with keys:
          `decoded_boxes`: A `float` tf.Tensor of shape
            [batch, num_raw_boxes, 4] representing all the decoded boxes.
          `decoded_box_scores`: A `float` tf.Tensor of shape
            [batch, num_raw_boxes] representing scores of all the decoded boxes.
    ...
    boxes, scores, attributes = self._decode_multi_level_outputs(
    ...
    with nms_context:
      ...
      if self._nms_version == 'batched':
        nmsed_boxes, nmsed_scores, nmsed_classes, valid_detections = \
            _generate_detections_batched(
                boxes, scores, self._pre_nms_score_threshold,
                self._nms_iou_threshold,
                self._max_num_detections)
        # Set `nmsed_attributes` to None for batched NMS.
        nmsed_attributes = {}
      elif self._nms_version == 'v1':
        values = _generate_detections_v1(
            boxes, scores,
            ...
            max_num_detections=self._max_num_detections,
            soft_nms_sigma=self._soft_nms_sigma)
        nmsed_boxes = values[0]
        nmsed_scores = values[1]
        nmsed_classes = values[2]
        valid_detections = values[3]
        nmsed_attributes = values[4]
      elif self._nms_version == 'v2':
        nmsed_boxes, nmsed_scores, nmsed_classes, valid_detections = \
            _generate_detections_v2(
                boxes, scores,
  ...
  def get_config(self):
    config = super(MultiLevelDetectionGenerator, self).get_config()
    config.update({
        'apply_nms': self._apply_nms,
        'pre_nms_top_k': self._pre_nms_top_k,
        'pre_nms_score_threshold': self._pre_nms_score_threshold,
