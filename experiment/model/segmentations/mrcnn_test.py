...
from experiment.model.decoders import fpn as fpn
from experiment.model.heads import dense_prediction_heads
from experiment.model.heads import instance_heads
from experiment.model.layers import detection_generator
from experiment.model.layers import mask_sampler
...
class MaskRCNNModelTest(parameterized.TestCase, tf.test.TestCase):
  ...
  def test_build_model(self, include_mask, use_separable_conv,
    ...
    if build_anchor_boxes:
      anchor_boxes = anchor.Anchor(
          ...
          anchor_size=3,
          image_size=(image_size, image_size)).multi_level_boxes
    ...
    with strategy.scope():
      if build_anchor_boxes:
        anchor_boxes = anchor.Anchor(
            ...
            aspect_ratios=aspect_ratios,
            anchor_size=anchor_size,
            image_size=image_size).multi_level_boxes
      else:
        anchor_boxes = None
  ...
  def test_serialize_deserialize(self, include_mask):
    ...
    new_model = mrcnn.MaskRCNNModel.from_config(config)
