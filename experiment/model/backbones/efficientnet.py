...
class BlockSpec():
  ...
  def __init__(self,
    ...
    self.in_filters = tf_utils.round_filters(in_filters, width_scale)
    self.out_filters = tf_utils.round_filters(out_filters, width_scale)
...
def block_spec_decoder(specs: List[Tuple[Any, ...]],
                       width_scale: float,
...
class EfficientNet(tf.keras.Model):
  ...
  def __init__(self,
    ...
    # Build stem.
    x = layers.Conv2D(
        filters=tf_utils.round_filters(32, width_scale),
        kernel_size=3,
        strides=2,
        use_bias=False,
        padding='same',
    ...
    x = self._norm(
        axis=bn_axis, momentum=norm_momentum, epsilon=norm_epsilon)(x)
    ...
    for i, spec in enumerate(decoded_specs):
      x = self._block_group(
          inputs=x, spec=spec, name='block_group_{}'.format(i))
      if spec.is_output:
    ...
    x = layers.Conv2D(
        filters=tf_utils.round_filters(1280, width_scale),
        kernel_size=1,
        strides=1,
        use_bias=False,
        padding='same',
  ...
  def _block_group(self,
    ...
    if spec.block_fn == 'mbconv':
      block_fn = nn_blocks.InvertedBottleneckBlock
    else:
      raise ValueError('Block func {} not supported.'.format(spec.block_fn))

    x = block_fn(
        in_filters=spec.in_filters,
        out_filters=spec.out_filters,
    ...
    for _ in range(1, spec.block_repeats):
      x = block_fn(
          in_filters=spec.out_filters,  # Set 'in_filters' to 'out_filters'.
          out_filters=spec.out_filters,
          expand_ratio=spec.expand_ratio,
          ...
          use_sync_bn=self._use_sync_bn,
          norm_momentum=self._norm_momentum,
          norm_epsilon=self._norm_epsilon)(x)
