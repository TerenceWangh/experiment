...
class Conv2DBNBlock(layers.Layer):
  ...
  def build(self, input_shape):
    if self._use_explicit_padding and self._kernel_size > 1:
      padding_size = tf_utils.get_padding_for_kernel_size(self._kernel_size)
      self._pad = layers.ZeroPadding2D(padding_size)
    self._conv_0 = layers.Conv2D(
        filters=self._filters,
        kernel_size=self._kernel_size,
        strides=self._strides,
...
def block_spec_decoder(
    ...
    divisible_by: int = 8, # Set to 1 for mobilenetv1.
  ...
  =========
  specs : dict
      The specification of block specs of a mobilenet version.
  filter_size_scale : float
      The multiplier for the filter size for all convolution ops. The value must
      be greater than zero. Typical usage will be to set this value in (0, 1) to
      reduce the number of parameters or computation cost of the model.
  divisible_by : int, default 8
      Ensuring all inner dimensions are divisible by this number.
  finegrain_classification_mode : bool, default True
      If True, the model will keep the last layer large even for small
      multipliers, following https://arxiv.org/abs/1801.04381.
  """
  ...
  if len(block_specs[0]) != len(block_spec_schema):
    raise ValueError('The block spec values {} do not match with the schema '
                     '{}'.format(block_specs[0], block_spec_schema))

  decoded_specs = []

  for s in block_specs:
    kws = dict(zip(block_spec_schema, s))
    decoded_specs.append(BlockSpec(**kws))

  # This adjustment applies to V2 and V3
  if (spec_name != 'MobileNetV1'
      and finegrain_classification_mode
      and filter_size_scale < 1.0):
    decoded_specs[-1].filters /= filter_size_scale # pylint: disable=annotation-type-mismatch

  for ds in decoded_specs:
    if ds.filters:
      ds.filters = tf_utils.round_filters(filters=ds.filters,
...
class MobileNet(tf.keras.Model):
  """Create a MobileNet family model."""
  ...
  def _get_divisible_by(self):
    return 1 if self._model_id == 'MobileNetV1' else self._divisible_by
  ...
  def _mobilenet_base(self,
    ...
    =========
    inputs : tf.Tensor
        The shape `[batch_size, height, width, channels]`.
    ...
    if len(input_shape) != 4:
      raise ValueError('Expected rank 4 input, was: {}.'.format(
          len(input_shape)))
    ...
    for i, block_def in enumerate(self._decoded_specs):
      ...
      net = layers.Activation('linear', name=block_name)(net)

      if block_def.is_output:
        endpoints[str(endpoint_level)] = net
        for k, t in intermediate_endpoints.items():
          endpoints['{}/{}'.format(str(endpoint_level), k)] = t
