...
from experiment.model.backbones import register_backbone_builder
...
class ResNet(tf.keras.Model):
  ...
  def __init__(self,
    ...
    ----------
    model_id : int
      The depth of ResNet backbone model.
    input_specs : tf.keras.layers.InputSpec
      The input tensor.
    depth_multiplier : float, default 1.0
      The depth multiplier to uniformly scale up all layers in channel
      size. This argument is also referred to as `width_multiplier` in (
      https://arxiv.org/abs/2103.07579).
    stem_type : str, default 'v0'
      The stem type of ResNet. If set to `v1`, use ResNet-D type stem (
      https://arxiv.org/abs/1812.01187).
    resnetd_shortcut : bool, default False
      Whether to use ResNet-D shortcut in downsampling blocks.
    replace_stem_max_pool : bool, default False
      Whether to replace the max pool in stem with a stride-2 conv.
    se_ratio : float, optional
      Ratio of the Squeeze-and-Excitation layer.
    init_stochastic_depth_rate : float, default 0.0
      The initial stochastic depth rate.
    scale_stem : bool, default True
      Whether to scale stem layers.
    ...
    kernel_initializer : str, default 'VarianceScaling'
      The kernel initializer of convolutional layers.
    kernel_regularizer : tf.keras.regularizers.Regularizer, optional
      The kernel regularizer of convolutional layers.
    bias_regularizer : tf.keras.regularizers.Regularizer, optional
      The bias regularizer of convolutional layers.
    bn_trainable : bool, default True
      Whether batch norm layers should be trainable.
  ...
  def _block_group(self,
    ...
    ----------
    inputs : tf.Tensor
      The size `[batch, channels, height, width]` for inputs.
    filters : int
      The number of filters for the first convolution of the layer.
    strides : int
      The stride to use for the first convolution of the layer. If greater
      than 1, this layer will downsample the input.
    block_fn : callable[..., tf.keras.layers.Layer]
      The type of block group. Either `nn_blocks.ResidualBlock` or
      `nn_blocks.BottleneckBlock`.
    block_repeats : int, default 1
      The number of blocks contained in the layer.
    stochastic_depth_drop_rate : float, default 0.0
      The drop rate of the current block group.
    name : str, default 'block_group'
    ...
    x = block_fn(
        ...
        bn_trainable=self._bn_trainable)(inputs)
...
@register_backbone_builder('resnet')
def build_resnet(
    input_specs: tf.keras.layers.InputSpec,
